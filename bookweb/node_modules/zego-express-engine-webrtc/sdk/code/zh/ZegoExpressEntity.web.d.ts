import { ZegoRTMEvent, ZegoUser } from "../../src/common/zego.entity";
/**
 * Note:
 *    创建流成功后拿到的 stream 为mediaStream对象,开发者可通过赋值给<video>或<audio>对象的srcObject属性进行渲染
 *    audioInput 和 videoInput 不指定时使用默认设备
 *    降噪、自动增益、回声消除三者默认为开启,若无特殊情况,建议客户保持这三项为 true
...
export interface ZegoRTCEvent {
     ...
     * 房间内流变化回调
     *
     * 收到流删除通知时，开发者可在 extendedData 中根据 stream_delete_reason 字段获取流删除的原因，code字段可能为如下值：1（用户主动停止推流）；  2（用户心跳超时）；  3（用户重复登录）；  4（用户被踢出）；  5（用户断线）；  6（被服务端移除）
     */
...
export interface ZegoWebPublishOption {
    ...
    extraInfo?: string;
    /**
     * 推流视频编码，只能传入 'VP8' (string) 或 'H264' (string)
     */
    videoCodec?: "VP8" | "H264";
}
/**
