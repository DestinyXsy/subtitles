import { ZegoWebRTC } from "../../src/webrtc/index";
import { ZegoExpressWebRTMEngine } from "../../src/common/zego.entity";
import { ZegoEvent, ZegoCapabilityDetection, ZegoDeviceInfos, ZegoLocalStreamConfig, ZegoMixStreamAdvance, ZegoMixStreamConfig, ZegoPublishStreamAudioConfig, ZegoPublishStreamConfig, ZegoServerResponse, ZegoWebPlayOption, ZegoWebPublishOption, ZegoCheckSingleType } from "./ZegoExpressEntity.web";
import { ClientUtil } from "../../src/util/client-util";
export declare class ZegoExpressEngine extends ZegoExpressWebRTMEngine {
    /**
     * 支持能力检测接口
     *
     * Note: 详情描述：音视频中很多功能只有较新版本的主流浏览器才支持，可以通过该接口来检测
     *
     ...
     * Note: 关注回调：无
     *
     * Note: 重点提示：初始化后立刻调用,建议在客户更换浏览器后,使用sdk前必须调用. 如果返回的errorInfo显示 Detection timeout 则是检测超时, 并不是浏览器不支持检测的能力类型.
     *
     * Note: 支持版本：1.0.0
     *
     * Note: 废弃时间：无
     ...
     *    channelCount 仅chrome 浏览器支持
     *   屏幕共享设置需要音频时，只推送系统声音，不推送麦克风声音
     * Note: 支持版本：1.0.0
     *
     * Note: 废弃时间： 无
     *
     ...
     *
     * Note: 详情描述：获取设备硬件信息，为控制硬件设备接口，提供设备id参数
     *
     * Note: 调用时机：初始化后
     *
     * Note: 限制频率：没有限制
     ...
     * 部分浏览器需要在调用createStream接口获取权限后,才可以获取到设备id;
     * safari不支持获取扬声器信息;
     *
     * Note: 支持版本：1.0.0
     *
     ...
     * Note: 关注回调：无
     *
     * Note: 重点提示：停止或恢复拉取音频流的前提是原始流必须有音频轨道，拉流时不能为纯视频
     *
     * Note: 支持版本：1.15.0
     *
     * Note: 废弃时间：无
     ...
     * Note: 关注回调：无
     *
     * Note: 重点提示：停止或恢复拉取视频流的前提是原始流必须有视频轨道，拉流时不能为纯音频
     *
     * Note: 支持版本：1.15.0
     *
     * Note: 废弃时间：无
     ...
     * 删除注册过的回调事件
     *
     * Note: 详情描述：ZegoEvent 包括了 [ZegoRTCEvent](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTCEvent)  与 [ZegoRTMEvent](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~javascript_web~interface~ZegoRTMEvent) 用于处理SDK主动通知开发者回调的接口，通过注册不同 event 的事件，可以收到不同功能的回调
     *
     * Note: 调用时机：注册后,退出房间前
     *
     * Note: 限制频率：无
     ...
     */
    setAudioConfig(localStream: MediaStream, constraints: ZegoPublishStreamAudioConfig): Promise<ZegoServerResponse>;
    /**
     * 创建流后可通过该接口调节采集音量
     *
     * 调节采集音量
     ...
     * Note: 关注回调：无
     *
     * Note: 重点提示：创建流成功后才能调用该接口，进行混音后仅对采集的音轨有效，混音部分音量请使用setMixingAudioVolume接口进行调节
     *
     * Note: 支持版本：1.0.0
     *
     * Note: 废弃时间： 无
     ...
     */
    setCaptureVolume(localStream: MediaStream, volume: number): Promise<ZegoServerResponse>;
    /**
     * 混流高级配置
     *
     ...
     * Note: 关注回调：无
     *
     * Note: 重点提示：Chrome 浏览器自 86 版本起本地音频标签设置为静音，拉流端也无法听到混入的背景音
     *
     * Note: 支持版本：1.18.0
     *
     * Note: 废弃时间： 无
     *
     * @param streamID 推流ID
     * @param volume 音量值，范围为 0～100，100表示原始音量
